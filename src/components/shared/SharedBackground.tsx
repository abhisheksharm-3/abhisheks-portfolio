...
const AnimatedPath = ({ className, pathD = SHARED_SVG_PATHS.default, delay = 0 }: AnimatedPathProps) => (
  <svg width="100" height="100" viewBox="0 0 100 100" fill="none" className={className}>
    <motion.path d={pathD} stroke="currentColor" strokeWidth="0.5" initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 2, delay }} />
  </svg>
);
...
const DecorativePaths = ({ isInView }: { isInView: boolean }) => (
  <>
    <motion.div initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ duration: 1, delay: 0.6 }} className="absolute left-[5%] top-[15%] text-primary/8">
      <AnimatedPath pathD={SHARED_SVG_PATHS.flowing} delay={0.6} />
    </motion.div>
    <motion.div initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ duration: 1, delay: 0.8 }} className="absolute right-[10%] bottom-[20%] rotate-180 text-primary/8">
 ...
 */
export const SharedBackground = ({ isInView, noiseFilterId }: { isInView: boolean; noiseFilterId: string }) => (
