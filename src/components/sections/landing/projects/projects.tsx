...
const HeaderContent = ({ headline, isInView }: { headline: string; isInView: boolean }) => (
  <div>
...
const ViewAllLink = ({ isInView }: { isInView: boolean }) => (
  <Link href="/projects" className="mt-7 sm:mt-0 group flex items-center text-sm text-foreground/60 hover:text-primary transition-colors">
    <motion.span initial={{ opacity: 0 }} animate={isInView ? { opacity: 1 } : {}} transition={{ duration: 0.5, delay: 0.7 }} className="relative font-medium">
      View all works
      <span className="absolute -bottom-1 left-0 w-0 h-[1px] bg-primary group-hover:w-full transition-all duration-300"></span>
    </motion.span>
    <motion.div initial={{ opacity: 0, x: -5 }} animate={isInView ? { opacity: 1, x: 0 } : {}} transition={{ duration: 0.5, delay: 0.8 }}>
...
const SectionHeader = ({ headline, isInView }: { headline: string; isInView: boolean }) => (
  <motion.div
...
const CTASection = ({ isInView }: { isInView: boolean }) => (
  ...
  >
    <Button variant="outline" size="lg" className="group border-primary/10 text-base px-8 py-6 rounded-xl hover:bg-primary/5 transition-colors relative overflow-hidden" asChild>
      <Link href="/contact" className="flex items-center">
        <motion.span className="font-medium tracking-wide relative z-10" whileHover={{ scale: 1.02 }} transition={{ duration: 0.2 }}>
          Let&apos;s collaborate
        </motion.span>
        <motion.div className="ml-3 h-6 w-6 rounded-full border border-primary/20 flex items-center justify-center relative z-10" whileHover={{ rotate: 45 }} transition={{ duration: 0.3 }}>
          <ArrowUpRight className="h-3 w-3 text-primary/70" />
        </motion.div>
        <motion.div className="absolute inset-0 bg-gradient-to-r from-primary/5 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500" initial={{ x: "-100%" }} whileHover={{ x: 0 }} transition={{ duration: 0.5 }} />
      </Link>
    </Button>
  </motion.div>
);
...
const ProjectCard = ({ project, index, isInView, isHovered, onHover, onLeave }: ProjectCardProps) => {
  const cardAnimation = {
    ...
    transition: {
      ...
      delay: PROJECT_ANIMATION_DELAYS.CARD_BASE + index * PROJECT_ANIMATION_DELAYS.CARD_STAGGER,
      ease: "easeOut" as const,
    },
  };
  return (
    <motion.div {...cardAnimation} onMouseEnter={onHover} onMouseLeave={onLeave} onFocus={onHover} onBlur={onLeave} className="relative">
      <Card className={`overflow-hidden border-primary/10 backdrop-blur-sm transition-all duration-300 py-0 ${isHovered ? 'shadow-lg border-primary/20' : 'shadow-md'}`}>
 ...
 */
export const Projects = ({ headline = "Selected Projects", cta = true }: ProjectsProps) => {
