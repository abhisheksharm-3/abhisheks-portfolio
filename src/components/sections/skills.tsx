...
import { Service, AnimatedPathProps } from "@/lib/types";

// Service data - centralized for easy maintenance
const SERVICES: Service[] = [
  ...
  },
  {
    ...
    skills: ["MongoDB", "Neo4j", "Firebase", "Appwrite"],
    level: "Proficient"
  },
  {
    icon: <Smartphone />,
...
// Reusable animated SVG path component
function AnimatedPath({ className, pathD }: AnimatedPathProps) {
...
// Background decorative elements - extracted for cleaner main component
function BackgroundElements({ isInView }: { isInView: boolean }) {
  return (
    <div className="absolute inset-0 pointer-events-none">
      ...
      {/* Asymmetrical grid pattern matching hero design */}
      ...
      {/* Animated decorative paths for visual interest */}
      <motion.div
        initial={{ opacity: 0 }}
      ...
      >
        <AnimatedPath pathD="M10,30 C20,50 40,10 50,40 S80,20 90,40" />
      </motion.div>
      <motion.div
        initial={{ opacity: 0 }}
        animate={isInView ? { opacity: 1 } : { opacity: 0 }}
      ...
      >
        <AnimatedPath pathD="M10,50 Q40,20 50,50 T90,30" />
      </motion.div>
    </div>
  );
}
...
// Individual skill card for the main services grid
...
// Main featured skills section with development approach
function FeaturedSkillsSection({ mainServices, isInView }: { mainServices: Service[]; isInView: boolean }) {
  return (
...
// Secondary skills displayed in a compact card grid
function SecondarySkillsGrid({ additionalServices, isInView }: { additionalServices: Service[]; isInView: boolean }) {
  return (
    <motion.div
      variants={ANIMATION_CONFIG.container}
      initial="hidden"
