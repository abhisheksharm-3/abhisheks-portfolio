...
export function ProjectDetailSidebar({ project }: { project: Project }) {
  ...
  return (
    <Card className="border-primary/10 backdrop-blur-sm overflow-hidden relative h-full">
      <motion.div variants={containerVariants} className="p-6 sm:p-8">
        ...
        <motion.div variants={containerVariants} className="space-y-4">
          {projectDetails.map((detail) => (
            <motion.div
              ...
              key={detail.label}
            >
              <span className="w-24 text-sm text-foreground/50 flex-shrink-0">
                {detail.label}:
              </span>
          ...
          {project.url && (
            <motion.div variants={itemVariants} className="flex-1">
              <Button
                variant="default"
                size="sm"
                className="group w-full bg-primary hover:bg-primary/90 text-primary-foreground"
                asChild
              >
                <a
                  href={project.url}
          ...
          {project.github && (
            <motion.div variants={itemVariants} className="flex-1">
              <Button
                variant="outline"
                size="sm"
                className="group w-full border-primary/10 hover:bg-primary/5 hover:border-primary/20"
                asChild
              >
                <a
                  href={project.github}
