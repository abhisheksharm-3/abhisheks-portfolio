...
const TimelineChart = ({ yearlyBreakdown }: { yearlyBreakdown: { year: string, count: number }[] }) => {
  const maxCount = useMemo(() => Math.max(...yearlyBreakdown.map(y => y.count)), [yearlyBreakdown]);
  
  return (
    <div className="border border-primary/10 rounded-lg p-4 bg-background/20 backdrop-blur-sm">
      <div className="text-xs text-foreground/50 mb-3 uppercase tracking-wider">Project Timeline</div>
      <motion.div variants={containerVariants} className="flex items-end justify-between gap-2 h-16">
        {yearlyBreakdown.slice(0, 5).map((year) => {
          const height = (year.count / maxCount) * 100;
          return (
            <motion.div key={year.year} variants={itemVariants} className="flex-1 flex flex-col items-center">
              <motion.div
              ...
              />
              <div className="text-xs text-foreground/40 font-mono">{year.year.slice(-2)}</div>
            </motion.div>
          );
        })}
      </motion.div>
    ...
    <motion.div variants={containerVariants} className="relative mt-24 mb-16">
      <motion.div variants={itemVariants} className="text-center mb-12">
        <div className="flex items-center justify-center mb-6">
          ...
          <span className="text-xs text-primary/60 uppercase tracking-wider font-light">Technical Profile</span>
        </div>
        <div className="h-[1px] w-12 bg-gradient-to-r from-transparent via-primary/30 to-transparent mx-auto" />
      </motion.div>

      <motion.div variants={containerVariants} className="grid lg:grid-cols-2 gap-12 max-w-4xl mx-auto">
        <motion.div variants={itemVariants} className="space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <StatCard value={`${localStats.diversityScore}%`} label="Diversity" icon={TrendingUp} />
            <StatCard value={localStats.activeProjects} label="Active" icon={Code2} />
          </div>
          <TimelineChart yearlyBreakdown={localStats.yearlyBreakdown} />
        </motion.div>

        <motion.div variants={itemVariants} className="border border-primary/10 rounded-lg p-6 bg-background/20 backdrop-blur-sm">
          <div className="text-center mb-4">
            <div className="text-sm text-foreground/70 mb-1">Technical Expertise</div>
            <div className="text-xs text-foreground/40">Proficiency across domains</div>
          </div>
          {radarData.length > 0 && (
            <ChartContainer config={{ value: { label: "Skills", color: colors.fill } }} className="mx-auto aspect-square max-h-[250px]">
                ...
                <ChartTooltip
                  ...
                  content={({ active, payload }) => active && payload?.length ? (
                    <div className="rounded-lg border border-primary/10 bg-background/95 backdrop-blur-sm p-3 shadow-xl">
                      <div className="text-sm font-medium text-foreground/90 mb-1">{payload[0].payload.fullName}</div>
                      <div className="text-xs text-foreground/50 font-mono">{Math.round(payload[0].payload.normalized)}% proficiency</div>
                    </div>
                  ) : null}
                />
                <PolarGrid stroke={colors.grid} />
                <PolarAngleAxis dataKey="technology" tick={{ fontSize: 10, fill: colors.text }} />
                <Radar dataKey="normalized" fill={colors.fill} fillOpacity={0.1} stroke={colors.fill} strokeWidth={2} />
              </RadarChart>
            </ChartContainer>
          )}
        </motion.div>
