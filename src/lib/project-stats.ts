...
export function calculateProjectStats(): ProjectStats {
  ...
  const projectTypes = new Set(projects.map(p => {
    if (p.tags.some(tag => ['Machine Learning', 'AI', 'TensorFlow', 'PyTorch'].includes(tag))) return 'AI/ML';
    if (p.tags.some(tag => ['Android', 'React Native', 'Flutter', 'iOS'].includes(tag))) return 'Mobile';
    if (p.tags.some(tag => ['Web3', 'Solidity', 'Blockchain', 'Ethereum'].includes(tag))) return 'Blockchain';
    if (p.tags.some(tag => ['NextJs', 'React', 'Vue', 'Angular'].includes(tag))) return 'Web';
    if (p.tags.some(tag => ['Python', 'FastAPI', 'Node.js', 'Express'].includes(tag))) return 'Backend';
    return 'Other';
  }));
  const diversityScore = Math.round((projectTypes.size / 6) * 100);

  const currentInternshipStatus = "Fresh off SDE Internship @ Stardom";
  const mostActiveYear = yearlyBreakdown.length > 0 ? yearlyBreakdown[0].year : currentYear.toString();
